...
STRUCTURED_PROMPTS_WITH_CONTEXT: Dict[str, str] = {
    ...
    "image": (
        "【强制要求】极致分析图片内容并生成高信息密度的RAG友好描述。\n\n"
        "【数据完整性强制约束】\n"
        "⚠️ 图片中有数据时，必须描述所有可见数据点，不得遗漏任何数值！\n"
        "⚠️ 遗漏任何数据点都将导致描述不完整，必须重新生成！\n"
        "⚠️ 即使字数限制，也必须优先保证数据完整性！\n\n"
        "图像信息：\n"
        "- 图像路径：{image_path}\n"
        "- 文档上下文：{context}\n\n"
        "【强制分析流程】\n"
        ...
        "第四步：验证数据完整性\n\n"
        "极致分析要求：\n"
        "1. **【强制】Description 数据完整性策略**：\n"
        "   - 开篇句：图片类型 + 主题实体 + 时间/地域 + 核心指标/用途（30-40字）\n"
        "   - 结构句：轴信息/系列构成/关键元素/布局特征（40-60字）\n"
        "   - 【强制】数据完整句：所有可见数值的完整枚举，确保不遗漏任何数据点（60-120字）\n"
        "   - 洞察句：极值/趋势/对比/意义或操作要点（40-60字）\n"
        "   - 关联句：与文档关系 + 应用场景（30-40字）\n"
        "   - 总字数：150-300字，【数据完整性绝对优先】\n\n"
        "2. **【强制】图表类数值完整策略**：\n"
        "   - 【必须】轴信息完整：x轴标题/值域/单位，y轴标题/单位/刻度，系列数量\n"
        "   - 【必须】全量数据枚举：每个系列的每个数据点都必须列出，使用紧凑格式\n"
        '   - 【必须】数据格式："时间：系列1 数值1/数值2/数值3；系列2 数值1/数值2/数值3 单位"\n'
        "   - 【必须】图例与标注：所有文字标注、箭头、高亮、特殊标记\n"
        "   - 【必须】单位处理：保留原始单位，提供标准单位换算\n"
        "   - 【必须】数据校验：饼图总和≈100%，系列长度对齐，数值单调性\n"
        '   - 【必须】不可读处理：无法识别的数值用"≈"标记并说明原因\n\n'
        "3. **【强制】非图表类功能完整策略**：\n"
        "   - 【必须】功能角色：证据/步骤说明/结构展示/界面演示/示例\n"
        "   - 【必须】关键元素：所有按钮/模块/节点/标注的位置和功能\n"
        "   - 【必须】操作要点：用户可执行的所有动作和步骤\n"
        "   - 【必须】应用场景：主要用途和目标用户\n"
        "   - 【必须】OCR文本：所有可识别的文字信息与实体\n\n"
        "4. **【强制】数据完整性检查清单**：\n"
        "   □ 是否统计了所有可见数据点数量？\n"
        "   □ 是否描述了每个系列的每个数据点？\n"
        "   □ 是否包含了所有轴信息（标题、单位、刻度）？\n"
        ...
        "   □ 是否验证了数据一致性（如饼图总和）？\n\n"
        "5. **【强制】示例参考**：\n"
        '   图表类示例："该柱状图展示某公司A/B产品在2020-2022年的营收变化。横轴为年份，纵轴为营收（亿元）。2020/2021/2022：A产品 12.3/15.8/18.1亿元；B产品 9.1/10.4/12.0亿元。A持续高于B，2022年总额达30.1亿元创峰值。"\n'
        "   非图表类示例：\"该界面截图展示TableParser应用的数据导入功能。主要包含蓝色主按钮'导入'、右上角齿轮'设置'图标、顶部导航栏等关键元素。用户可通过该界面完成数据导入操作和基础配置设置。\"\n\n"
        "6. **Keywords 精准化策略**：\n"
        "   - 类型词：1-2个，细粒度类型 + 主要别名（柱状图/bar chart）\n"
        "   - 主题词：1-2个，核心实体 + 主要别名（公司名/产品名）\n"
        "   - 指标词：1-2个，核心指标 + 主要同义词（营收/收入/亿元）\n"
        "   - 时间词：1个，时间范围或关键时间点（2020-2022/2021年）\n"
        "   - 对象词：1-2个，主要系列/类别/对比对象\n"
        "   - 关系词：1-2个，核心趋势/对比/关系词（增长/最高/占比）\n"
        "   - 视觉词：1-2个，主要颜色/布局/样式特征\n"
        "   - 功能词：1-2个，用途/动作/场景词（设置/导入/对比）\n"
        "   - 总数量：8-15个，按重要性排序\n\n"
        "7. **【强制】RAG检索优化**：\n"
        "   - 描述结构化：便于向量化检索的层次化信息\n"
        "   - 关键词权重：重要词靠前，确保高权重检索\n"
        "   - 同义覆盖：确保主要表达方式都能检索到\n"
        "   - 事实化表达：便于精确匹配\n"
        "   - 【必须】数据可核验：所有数值都可以被验证和查询\n\n"
        "【强制生成规范】：\n"
        "- description：150-300字，【数据完整性绝对优先】，5个层次结构\n"
        "  * 图表类：开篇(类型+主题+时间+指标) + 结构(轴+系列) + 【强制】数据(所有数值完整枚举) + 洞察(极值+趋势) + 关联(文档关系)\n"
        "  * 非图表类：开篇(类型+主题+功能) + 结构(元素+布局) + 功能(主要作用) + 应用(场景+操作) + 关联(文档关系)\n"
        "- keywords：8-15个，8个维度精选，权重排序，同义覆盖\n"
        "- data_insights：3-4条事实句，【必须】包含具体数值/功能/意义\n"
        "- visual_elements：3-5个视觉锚点，便于检索\n"
        "- searchable_queries：6-8条，核心查询意图\n\n"
        "【强制质量控制】：\n"
        "- 【绝对必须】数据完整性：确保所有可见数据点都被描述\n"
        "- 【绝对必须】字数控制：description在150-300字内必须完整描述数据\n"
        ...
        "【强制输出格式】：\n"
        "{{\n"
        '  "description": "【强制】极致完整描述：开篇(类型+主题+时间+指标/用途) + 结构(轴/系列/元素) + 【强制】数据(所有数值完整枚举) + 洞察(极值/趋势/对比/意义) + 关联(文档关系+应用场景)",\n'
        '  "keywords": ["类型词", "主题词", "指标词", "时间词", "对象词", "关系词", "视觉词", "功能词"],\n'
        ...
        '  "data_insights": ["【强制】事实句1：具体数值/功能/意义", "【强制】事实句2：关键发现/操作要点", "【强制】事实句3：业务洞察/应用场景"],\n'
